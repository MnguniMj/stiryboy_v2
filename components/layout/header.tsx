    ...
    <header className="sticky top-0 z-50 bg-amazon-navy text-white shadow-lg">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-3 py-3 sm:px-5">
        ...
        <Link
          ...
          aria-label="Amazon India clone home"
        >
        ...
        <form
          onSubmit={onSearch}
          className="hidden flex-none overflow-hidden rounded-md border-2 border-transparent bg-white focus-within:border-amazon-orange md:flex"
        >
          <select
            ...
            onChange={(event) => {
              if (event.target.value !== "All") {
                router.push(
                  `/search?category=${encodeURIComponent(event.target.value)}`,
                );
              }
            }}
          >
          ...
          <button
            type="submit"
            className="flex w-12 items-center justify-center bg-amazon-gold text-slate-950 hover:bg-[#f5a742]"
            aria-label="Search"
          >
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          ...
          {session?.user ? (
            <div className="group relative hidden sm:block">
              ...
              <div className="invisible absolute right-0 top-full w-44 translate-y-2 rounded-md border border-slate-200 bg-white p-2 text-slate-900 opacity-0 shadow-glow transition group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 dark:border-white/10 dark:bg-slate-900 dark:text-white">
                <Link
                  className="block rounded px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-white/10"
            ...
            <Link
              href="/login"
              className="hidden rounded-md px-3 py-2 text-sm font-semibold hover:bg-white/10 sm:block"
            >
              Sign in
            </Link>
          )}
          ...
          <Link
            href="/cart"
            className="relative flex items-center gap-1 rounded-md px-2 py-2 font-semibold hover:bg-white/10"
            aria-label="Cart"
          >
      ...
      <div className="px-10 pb-3 md:hidden">
        <form
          onSubmit={onSearch}
          className="flex overflow-hidden rounded-md border-2 border-transparent bg-white focus-within:border-amazon-orange"
        >
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
      ...
      <div className="border-t border-white/10 bg-amazon-blue">
        <nav className="mx-auto flex max-w-7xl items-center gap-5 overflow-x-auto px-4 py-2 text-sm font-semibold sm:px-5 no-scrollbar">
          <Link
            href="/search"
            className="flex shrink-0 items-center gap-1 hover:text-amazon-gold"
          >
          ...
          {categories.map((category) => (
            <Link
              ...
              className="shrink-0 hover:text-amazon-gold"
            >
              {category}
            </Link>
          ))}
            ...
            {session?.user ? (
              <>
                ...
                <button
                  type="button"
                  onClick={() => signOut({ callbackUrl: "/" })}
                  className="rounded-md bg-white/10 px-3 py-2 text-left"
                >
                  Logout
                </button>
              </>
